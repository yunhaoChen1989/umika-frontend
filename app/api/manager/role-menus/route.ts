...
export async function GET(request: NextRequest) {
  ...
  const url = new URL(`${backendBaseUrl}/role-menus`);
...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${backendBaseUrl}/role-menus`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
...
export async function DELETE(request: NextRequest) {
  ...
  if (!roleId || !menuId) {
    return NextResponse.json({ message: "roleId and menuId are required." }, { status: 400 });
  }

  const response = await fetch(`${backendBaseUrl}/role-menus/${roleId}/${menuId}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token}`,
    },
